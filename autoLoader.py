...
#     except: raise
  ...
  def run(self, fitsfile, number=0, count=0):
    ...
    # Check that the value of the FITS header field makes sense - iterating
    # over the available HDUs - abort after first match
    matched = False
    hdu = 0
    hdu_max = (len(infile)-1)
    while ((not matched) and (hdu <= hdu_max)):
      try: 
        headerval = infile[hdu].header[thisrule.fitshead.upper()]
	matched = True
      except KeyError: pass
      hdu = hdu + 1

    if not matched: raise RuleError, "In rule %i: FITS header %s not found in %s" % (number, thisrule.fitshead.upper(), fitsfile)

    # Check that the FITS header has a value (to test against)
    if headerval == None: raise RuleError, "In rule %i: FITS header '%s' has no value in %s" % (number, thisrule.fitshead.upper(), fitsfile)
...
class window:
  ...
  def update(self):
    ...
    """
       Update ruleset contents from configuration object; useful after
       modifying rules outside the autoLoader, for example when reading
       rules from disk.
    """
  ...
  def loadConfig(self, infile=None):
    ...
    if infile:
      ...
      config.load_raw(infile, optionlist=['autoloadrules'])

      # Update ruleset content and refresh frame with new values
      self.update()
      self.reset()
