...
def findtraceord(infile, trace='yes', reference="", interactive='yes'):
  ...
  # Save a copy of the aperture definition file (no errors reported)
  apfile = irafDbNames.apname(infile)
  try: shutil.copy(apfile, apfile+'_prev')
  except: pass
  ...
  if interactive == 'yes':
    gwm.window("SALT-HRS-Tool - Calibration - Order Definition - ApFind")
  else:
    iraf.apparams.ansedit='NO'

  # Call the echelle.apfind task through PyRAF
  # (Workaround using 'sleep' commands) - thanks John!
  sleep(1.5)
  ...
  if interactive == 'yes': gwm.delete(gwm.getActiveWindowName())
...
def extractspec(infile, outfile, reffile, clean='yes'):
...
###
  ...
  iraf.splot(images=infile, line=order, band=1, mode='h')
