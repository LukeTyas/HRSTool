...
class window(Frame):
  ...
  def loadConfig(self, infile=None):
    ...
    if infile:
      ...
      config.load(infile, optionlist=self.optionlist, ignore_options=['currentmode'])
  ...
  def resetConfig(self):
    ...
    if answer.num == 0:
      ...
      # Store the current reduction mode in a variable
      currentmore = config.options['currentmode'].value
      
      # Restore the config options to the old values in oldoptions
      for key in self.optionlist:
        config.options[key] = copy.deepcopy(self.oldoptions[key])

      # Make sure we keep the current reduction mode
      # (because otherwise the screen might show something else than
      #  actually is loaded)
      config.options['currentmode'].value = currentmode
