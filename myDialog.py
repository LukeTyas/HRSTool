...
# Persistent global variable that will contain FITS header values
fileheaderlist = dict()
...
#       self.files.bind('<Double-ButtonRelease-1>', self.files_double_event)
    ...
    def getFITShead(self, file, cardname):
    	"""Extract value of FITS headers out of a file and stor in global variable."""

        cardval = ""
	if not fileheaderlist.has_key(file):
	  fileheaderlist[file] = dict()
	if not (cardname in fileheaderlist[file].keys()):
	  try:
	    img = pyfits.open(file)
	    fileheaderlist[file][cardname] = img[0].header[cardname]
	    img.close()
	  except: pass
	# Brace the next statement with try/except, because there filelist
	# might contain non-FITS files, for which there will be no header value
	try:
	  cardval = fileheaderlist[file][cardname]
	except: pass
	return cardval
...
def GetInteger(parent=None, prompt="", **opts):
  ...
  # Next line is not needed, since tkSimpleDialog will force correct answer, and
  # the 'cancel' button generates 'None'.
  # if not result: result = None
  return result
